...
%
Time_ABS = sldemo_absbrake_output.get('Sd').Values.Time; % Save the ABS results
Stop_ABS = sldemo_absbrake_output.get('Sd').Values.Data;
...
evalc('sim(''sldemo_absbrake'')'); %this code is not shown in the example
%% 
Time_no_ABS = sldemo_absbrake_output.get('Sd').Values.Time; %save the non-ABS results
Stop_no_ABS = sldemo_absbrake_output.get('Sd').Values.Data;
